...
Next, go ahead and put your Markdown documents (`.md` or `.mdx`) in `pages/`. Documents in `content/` automatically become pages with URLs based on their path relative to `pages/`. For example, if you create a `pages/components/box.md` file, Doctocat will use that file to create a `/components/box` page.
